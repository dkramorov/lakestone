	...
	protected function validateForm() {
		...
		if (isset($this->request->post['filter'])) {
			foreach ($this->request->post['filter'] as $filter_id => $filter) {
				if ( ! preg_match('/^[A-z0-9_.-]{1,50}$/', $filter['filter_tag']) ) {
					$this->error['filter'][$filter_id]['filter_tag'] = $this->language->get('error_tag');
					continue;
				}
				$f = $this->model_catalog_filter->askFilterTag($filter['filter_tag']);
				if (
					sizeof($f) > 0 and
					$filter['filter_id'] != $f['filter_id']
				) {
					$this->error['filter'][$filter_id]['filter_tag'] = $this->language->get('error_tag_uniq');
				}
