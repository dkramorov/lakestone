...
class ControllerCatalogReview extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($this->request->get['filter_review_type'])) {
				$url .= '&filter_review_type=' . urlencode(html_entity_decode($this->request->get['filter_review_type'], ENT_QUOTES, 'UTF-8'));
			}
    ...
    if (isset($this->request->get['filter_review_type'])) {
      $filter_review_type = $this->request->get['filter_review_type'];
    } else {
      $filter_review_type = null;
		...
		$filter_data = array(
			'filter_review_type' => $filter_review_type,
		...
		$data['filter_review_type'] = $filter_review_type;
	...
	protected function getForm() {
		...
		$data['entry_date_added'] = $this->language->get('entry_date_added');
		$data['entry_review_type'] = $this->language->get('entry_review_type');
		$data['entry_status'] = $this->language->get('entry_status');
		...
		if (isset($this->error['rating_photo'])) {
			$data['error_rating_photo'] = $this->error['rating_photo'];
		} else {
			$data['error_rating_photo'] = '';
		}

		if (isset($this->error['rating_description'])) {
			$data['error_rating_description'] = $this->error['rating_description'];
		} else {
			$data['error_rating_description'] = '';
		}
    ...
    $this->load->model('tool/image');
		...
		if (isset($this->request->post['rating_photo'])) {
			$data['rating_photo'] = $this->request->post['rating_photo'];
		} elseif (!empty($review_info)) {
			$data['rating_photo'] = $review_info['useful_photo'];
		} else {
			$data['rating_photo'] = '';
		}

		if (isset($this->request->post['rating_description'])) {
			$data['rating_description'] = $this->request->post['rating_description'];
		} elseif (!empty($review_info)) {
			$data['rating_description'] = $review_info['useful_description'];
		} else {
		...
		if (isset($this->request->post['answer_status'])) {
			$data['answer_status'] = $this->request->post['answer_status'];
		} elseif (!empty($review_info)) {
			$data['answer_status'] = $review_info['answer_status'];
		} else {
			$data['answer_status'] = '';
		}

		if (isset($this->request->post['review_type'])) {
			$data['review_type'] = $this->request->post['review_type'];
		} elseif (!empty($review_info)) {
			$data['review_type'] = $review_info['review_type'];
		} else {
			$data['review_type'] = '';
		}
		
		$data['images'] = [];
		foreach ($this->model_catalog_review->getReviewImages($this->request->get['review_id']) as $image) {
		  $data['images'][] =[
	...
	protected function validateForm() {
		...
		if ($this->request->post['review_type'] < 2) {
			if (!isset($this->request->post['rating']) || $this->request->post['rating'] < 0 || $this->request->post['rating'] > 5) {
				$this->error['rating'] = $this->language->get('error_rating');
			}

			if (!isset($this->request->post['rating_photo']) || $this->request->post['rating_photo'] < 0 || $this->request->post['rating_photo'] > 5) {
				$this->error['rating_photo'] = $this->language->get('error_rating_photo');
			}

			if (!isset($this->request->post['rating_description']) || $this->request->post['rating_description'] < 0 || $this->request->post['rating_description'] > 5) {
				$this->error['rating_description'] = $this->language->get('error_rating_description');
			}
		}
