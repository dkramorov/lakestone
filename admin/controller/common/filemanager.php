...
class ControllerCommonFileManager extends Controller {
	public function index() {
		...
		if (!empty($this->request->get['directory'])) {
		  $dir = preg_replace('@(^/?catalog/?)|(^/)|(\*)|(\.\.)|(/$)@', '', $this->request->get['directory']);
			$directory = realpath(DIR_IMAGE . 'catalog/' . $dir);
		} else {
		...
		if ($directory) {
			...
			// Get files
			$files = glob_brace($directory . '/' . $filter_name . '*.{jpg,jpeg,png,gif,svg,webp,mp4,JPG,JPEG,PNG,GIF,SVG,WEBP,MP4}');
...
//    d($images);
    ...
    if (!empty($directory)) {
      $data['heading_title'] = $this->language->get('heading_title') . ': ' . $dir;
    } else {
