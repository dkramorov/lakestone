...
class ControllerSettingSetting extends Controller {
	...
	public function index() {
		...
		if (isset($this->error['error_product_title'])) {
			$data['error_product_title'] = $this->error['config_product_title'];
		} else {
			$data['error_product_title'] = '';
		}

		if (isset($this->error['error_product_description'])) {
			$data['error_product_description'] = $this->error['config_product_description'];
		} else {
			$data['error_product_description'] = '';
		}

		if (isset($this->error['error_category_title'])) {
			$data['error_category_title'] = $this->error['config_category_title'];
		} else {
			$data['error_category_title'] = '';
		}

		if (isset($this->error['error_category_description'])) {
			$data['error_category_description'] = $this->error['config_category_description'];
		} else {
			$data['error_category_description'] = '';
		}
		...
		// Иконки для аксессуаров ==========================
		if (isset($this->request->post['config_accessory_link'])) {
			$data['config_accessory_link'] = $this->request->post['config_accessory_link'];
		} else {
			$data['config_accessory_link'] = $this->config->get('config_accessory_link');
		}
    
    $svgs = $this->config->get('icons')['accessory_links'];

		foreach ($data['config_accessory_link'] as $k => $v) {
			//$v['icon'] = str_replace('"',"'",html_entity_decode());
      
      $v['placeholder'] = 'data:image/svg+xml;utf8,' . str_replace('"', '\'', array_pop($svgs));
      if (!empty($v['icon'])) {
        $v['icon'] = $this->model_tool_image->resize($v['icon'], 64, 64);
      } else {
//        $v['icon'] = $v['placeholder'];
      }
			$data['config_accessory_link'][$k] = $v;
		}
		// =================================================
		...
		if (isset($this->request->post['config_product_title'])) {
			$data['config_product_title'] = $this->request->post['config_product_title'];
		} else {
			$data['config_product_title'] = $this->config->get('config_product_title');
		}
		...
		if (isset($this->request->post['config_category_description'])) {
			$data['config_category_description'] = $this->request->post['config_category_description'];
		} else {
			$data['config_category_description'] = $this->config->get('config_category_description');
		}
