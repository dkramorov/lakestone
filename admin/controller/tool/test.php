...
class ControllerToolTest extends Controller {
	...
	public function index() {
		...
		$this->response->addheader('Content-Disposition: attachment; filename="all_clients_email_' . date('Y-m-d_H-i-s', time()) . '.csv"');
		$this->response->addheader('Content-Transfer-Encoding: binary');
		
		$mails = '';
		$query = $this->db->query("SELECT `email` FROM `" . DB_PREFIX . "order` GROUP BY `email`");
		foreach ( $query->rows as $row ) {
			$mails .= $row['email'] . ";\n";
		}
		
		$this->response->setOutput($mails);
		return;
