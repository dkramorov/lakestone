...
class ModelCatalogFilter extends Model {
	public function addFilter($data) {
		...
		if (isset($data['filter'])) {
			foreach ($data['filter'] as $filter) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "filter SET filter_group_id = '" . (int)$filter_group_id . "', filter_tag = '" . $this->db->escape($filter['filter_tag']) . "', sort_order = '" . (int)$filter['sort_order'] . "'");
	...
	public function editFilter($filter_group_id, $data) {
		...
		if (isset($data['filter'])) {
			foreach ($data['filter'] as $filter) {
				if ($filter['filter_id']) {
					$this->db->query("INSERT INTO " . DB_PREFIX . "filter SET filter_id = '" . (int)$filter['filter_id'] . "', filter_tag = '" . $this->db->escape($filter['filter_tag']) . "', filter_group_id = '" . (int)$filter_group_id . "', sort_order = '" . (int)$filter['sort_order'] . "'");
				} else {
					$this->db->query("INSERT INTO " . DB_PREFIX . "filter SET filter_group_id = '" . (int)$filter_group_id . "', filter_tag = '" . $this->db->escape($filter['filter_tag']) . "', sort_order = '" . (int)$filter['sort_order'] . "'");
				}
	...
	public function getFilters($data) {
		...
		if (!empty($data['filter_tag'])) {
			$sql .= " AND f.filter_tag LIKE '" . $this->db->escape($data['filter_tag']) . "%'";
		}
