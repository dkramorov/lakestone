          ...
          <table id="filter" class="table table-striped table-bordered table-hover">
            <thead>
              <tr>
                ...
                <td class="text-right required"><?php echo $entry_tag; ?></td>
            ...
            <tbody>
              ...
              <tr id="filter-row<?php echo $filter_row; ?>">
                <td class="text-left" style="width: 50%;"><input type="hidden" name="filter[<?php echo $filter_row; ?>][filter_id]" value="<?php echo $filter['filter_id']; ?>" />
                  <?php foreach ($languages as $language) { ?>
                  <div class="input-group"><span class="input-group-addon"><img src="language/<?php echo $language['code']; ?>/<?php echo $language['code']; ?>.png" title="<?php echo $language['name']; ?>" /></span>
                ...
                <td class="text-left">
                  <div class="input-group">
                    <input type="text" name="filter[<?php echo $filter_row; ?>][filter_tag]" value="<?php echo $filter['filter_tag']; ?>" placeholder="<?php echo $entry_tag; ?>" id="input-filter-tag" class="form-control" />
                  </div>
                  <?php if (isset($error_filter[$filter_row]['filter_tag'])) { ?>
                  <div class="text-danger"><?php echo $error_filter[$filter_row]['filter_tag']; ?></div>
                  <?php } ?>
                </td>
...
function addFilterRow() {
	html  = '<tr id="filter-row' + filter_row + '">';	
        html += '  <td class="text-left" style="width: 50%;"><input type="hidden" name="filter[' + filter_row + '][filter_id]" value="" />';
	<?php foreach ($languages as $language) { ?>
	...
	html += '  </td>';
        ...
        html += '<div class="input-group">';
        html += '<input type="text" name="filter[' + filter_row + '][filter_tag]" value="" placeholder="<?php echo $entry_tag; ?>" id="input-filter-tag" class="form-control" />';
        html += '</div>';
        html += '</td>';
