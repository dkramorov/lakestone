<div id="filemanager" class="modal-dialog modal-lg">
  <div class="modal-content">
    ...
    <div class="modal-body">
      ...
      <hr />
      <?php foreach (array_chunk($images, 3) as $image) { ?>
      <div class="row">
        ...
        <div class="col-sm-4 col-xs-12 text-center">
          ...
          <a href="<?php echo $image['href']; ?>" class="preview">
<!--            <img src="" alt="<?php echo $image['name']; ?>" title="<?php echo $image['name']; ?>" />-->
            <video controls width="250">
              <source src="<?=$image['href']?>" type="video/mp4">
            </video>
            <div>Установить этот ролик</div>
          </a>
          <label>
            <input type="checkbox" name="path[]" value="<?php echo $image['path']; ?>" />
            <?php echo $image['name']; ?></label>
...
$('a.preview').on('click', function(e) {
    e.preventDefault();
    let $vid = $('<video controls width="250"></video>');
    $vid.append('<source src="' + $(this).attr('href') + '" type="video/mp4">');
    <?php if ($thumb) { ?>
        $('#<?php echo $thumb; ?>').find('img').replaceWith($vid);
    <?php } ?>
