<?php
class ControllerCheckoutPreorder extends Controller {
    ...
    public function index() {
        ...
        if ( isset($this->request->post['product_id']) ) {
            ...
            $this->session->data['payment_address'] = $this->session->data['shipping_address'] = array(
                    ...
                    'lastname'		=> '',
                    'company'		=> '',
                    'address_1'		=> 'Предзаказ, без адреса',
                    'address_2'		=> '',
                    'city'		=> '',
                    'postcode'		=> '',
                    'zone'		=> '',
                    'country'		=> '',
                    'address_format'	=> '',
                    'custom_field'	=> 'Preorder',
            );
            $this->session->data['vouchers'] = '';
            $this->session->data['comment'] = 'Preorder';
            $this->session->data['payment_method'] = array(
                    'code'		=> 'cod',
                    'title'		=> 'Предзаказ, не определено',
            );
...
/*	    $this->load->model('extension/module/retailcrm');
	    $res = $this->model_extension_module_retailcrm->createOrder(array(
            ...
            $json['text'] = 'Cпасибо за предзаказ, с Вами свяжется менеджер магазина';
            ...
            $json['text'] = 'Функция презаказа не работает, пожалуйста, позвоните нам по телефону';
            $this->response->addHeader('Content-Type: application/json');
            $this->response->setOutput(json_encode($json));
