...
class ControllerExtensionFeedMarketYml2 extends Controller {
  ...
  public function index() {
    ...
    if ($this->config->get('dev')) {
      ...
      $XML_str = $this->cache->get('market_yml2');
      $Limit = -1;
    }
    if (php_sapi_name() == 'cli') {
      $this->load->model('tool/status');
      $this->model_tool_status->start('market_yml2');
      $XML_str = false;
    }
...
//			$XDate->value=strftime('%Y-%m-%d %H:%M:%S');
      ...
      foreach ($this->model_catalog_product->getProducts() as $product) {
        ...
        $XOffer->appendChild($this->XML->createElement('barcode', $product['ean']));
      ...
      $this->cache->set('market_yml2', $XML_str);
      
    }
  
    if (php_sapi_name() == 'cli') {
      $this->model_tool_status->done('market_yml2', 1);
    } else {
