...
class ControllerInformationContact extends Controller {
	...
	public function index() {
		...
		$this->document->setTitle('Контакты LAKESTONE: адрес, карта проезда, телефон');
		$this->document->setDescription('Полные контактные данные, адрес, карта проезда, телефон LAKESTONE - интернет-магазин кожаных аксессуаров');
		$this->document->addLink($this->url->link('information/contact'), 'canonical');

		if ( $this->request->server['REQUEST_METHOD'] == 'POST' ) {
			$json = array();
			if (
				isset($this->request->post['json']) and
				$this->request->post['json'] == 'true'
			) {
				$this->request->post['email'] = $this->config->get('config_email');
				$this->request->post['name'] = 'Администратор магазина';
				if ( ! $this->validate() ) {
			...
			if ( $json ) {
				$json['text'] = 'Cпасибо за сообщение';
