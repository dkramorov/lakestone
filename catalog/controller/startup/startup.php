...
class ControllerStartupStartup extends Controller {
    ...
    if (defined('DEV_DEBUG'))
      $this->config->set('debug', DEV_DEBUG);
    else
    ...
    if (isset($this->request->get['store_id'])) {
		...
		// AdmiTag
		if (
		    isset($this->request->get['admitad_uid'])
        and preg_match('/[a-z0-9]{32}/', $this->request->get['admitad_uid'])
		) {
		  setcookie('admitad_uid', $this->request->get['admitad_uid'], time() + 3600*24*30);
			$this->session->data['admitad_uid'] = $this->request->get['admitad_uid'];
		} elseif (
		    isset($_COOKIE['admitad_uid'])
        and preg_match('/[a-z0-9]{32}/', $_COOKIE['admitad_uid'])
    ) {
      $this->session->data['admitad_uid'] = $_COOKIE['admitad_uid'];
    }
