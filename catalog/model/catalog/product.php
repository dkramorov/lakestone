...
class ModelCatalogProduct extends Model {
	...
	public function getAccessoriesLinks($category_id) {
    ...
    $svgs = $this->config->get('icons')['accessory_links'];
		...
		foreach ($tmp as $k=>$v){
		  ...
		  if (empty($v['icon'])) {
		    $icon = 'data:image/svg+xml;utf8,' . str_replace('"', '\'', $placeholder);
      } else {
		    $icon = '/image/' . html_entity_decode($v['icon']);
      }
			$icons[$k] = [
				'name' => $v['name'],
				'href' => $v['href'],
...
/*
		$query = $this->db->query("SELECT
			...
			`name`,
			`href`
		FROM " . DB_PREFIX . "accessories_icons ORDER BY 1 ASC");
...
*/
		...
		do {
			...
			if ($query->num_rows > 0) {
				...
				$category_id = $query->row['parent_id'] ?? false;
			}
		} while (empty($links) && $category_id);

		if (!empty($links)) {
			foreach ($links as $v) {
				foreach (['name', 'href'] as $key) {
					if (!empty($v[$key])) {
						$icons[$v['icon_id']][$key] = $v[$key];
					}
				}
			}
		}
	...
	public function getProducts($data = array()) {
		...
		if (isset($data['order']) && ($data['order'] == 'DESC')) {
			...
			$sql .= " DESC, LCASE(pd.yml_model) DESC";
		} else {
			// $sql .= " ASC, LCASE(pd.name) ASC";
			$sql .= " ASC, LCASE(pd.yml_model) ASC";
		}
	...
	public function getProduct($product_id, $status = true) {
		$sql = "SELECT DISTINCT *, pd.name AS name, p.image,m.name AS manufacturer,
			...
			(SELECT COUNT(*) AS total
				...
				AND r2.status = '1'
				AND r2.review_type = '0'
				GROUP BY r2.product_id
			) AS reviews,
			(SELECT COUNT(*) AS total
				FROM " . DB_PREFIX . "review r3
				WHERE r3.product_id = p.product_id
				AND r3.status = '1'
				AND r3.review_type = '1'
				GROUP BY r3.product_id
			) AS questions,
			p.sort_order
		...
		if ($query->num_rows) {
			return array(
				...
				'name' => $query->row['name'],
				'yml_name' => $query->row['yml_name'],
				'yml_model' => $query->row['yml_model'],
				'short' => $query->row['short_description'],
	...
	public function getProductImages($product_id, array $position = []) {
	  $where_addon = '';
	  if (!empty($position)) {
	    $where_addon = ' AND position IN (' . implode(',', $position) . ')';
    }
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_image WHERE product_id = '" . (int)$product_id . "'" . $where_addon . " ORDER BY sort_order ASC");
	...
	public function getCategories($product_id) {
		$query = $this->db->query("SELECT p2c.* FROM " . DB_PREFIX . "product_to_category p2c LEFT JOIN " . DB_PREFIX . "category_description as cd ON (p2c.category_id = cd.category_id) WHERE p2c.product_id = '" . (int)$product_id . "' ORDER BY LENGTH(cd.yml_path) DESC");
	...
	public function getTotalProducts($data = array()) {
		...
		if (!empty($data['filter_special'])) {
			$sql .= " AND ps.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "'
