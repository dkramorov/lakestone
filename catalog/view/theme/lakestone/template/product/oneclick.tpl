<div class="oneclick_order">
  <div class="product_purchase modal-default">
    ...
    <div class="customers_form">
      ...
      <div class="buttons">
        <button onClick="oneclick_send(this)" class="btn btn-red">Оформить заказ</button>
        <button class="btn btn-default" data-dismiss="modal" type="button">Отмена</button>
      </div>
    </div>
    <p class="disclaimer text-justify">Нажимая на кнопку <q>Оформить заказ</q>, вы принимаете условия <a class="blue" target="_blank" href="/publichnaya-oferta">Публичной оферты</a></p>
  </div>
</div>
<script>
  ...
  if ( ok ) {
    ...
    var res = $.ajax({
      ...
      success: function(d) {
        console.log(d)
        try {
          DataLayerPush_1click(d.order_id)
        } catch (e) {
          ErrorLog(new MyError('an error in oneclick_send', e))
        }
        cf.empty()
      ...
      complete: function() {
        cf.append('<div class="buttons b1"><button class="btn bt-default" type="link" data-dismiss="modal"><span aria-hidden="true">Закрыть</span></button></div>')
      }
    })
  } else {
    var m = $('#error-message')
...
var DataLayerPush_1click = function(order_id) {
