...
class Image {
  ...
  public function autoRotate() {
    ...
    if (!$exif or empty($exif['Orientation'])) return;
    ...
    switch ($exif['Orientation']) {
      ...
      case 4:
        $this->rotate(180)->mirror();
        break;
      
      case 5:
        $this->rotate(270)->mirror();
        break;
...
//		  $extension = 'webp';
        ...
        $res = imagewebp($this->image, $file);
			} else {
        $res = imagejpeg($this->image, $file);
      }
    ...
    if ($scale == 1 && $scale_h == $scale_w && $this->mime != 'image/png') {
		...
		if ($default == 'ceil') {
      $ypos = abs($ypos) / $scale_h;
      $xpos = abs($xpos) / $scale_w;
      imagecopyresampled($this->image, $image_old, 0, 0, $xpos, $ypos, $new_width, $new_height, $this->width, $this->height);
    } else {
  ...
  public function mirror() {
    ...
    $this->image = imageflip($this->image, IMG_FLIP_BOTH);
